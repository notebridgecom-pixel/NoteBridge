...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  React.useEffect(() => {
    if (initialWithdrawals) setWithdrawals(initialWithdrawals);
  }, [initialWithdrawals]);
  ...
  const handleRequestChanges = (noteId: string) => {
    ...
    setActionSuccess(`Requested changes for Note #${noteId}. Logged in Security & Audit Ledger.`);
    setTimeout(() => setActionSuccess(''), 4000);
  };
              ...
              {pendingNotes.map((note) => (
                ...
                >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-emerald-800 bg-emerald-100 px-3 py-1 rounded-full text-xs">
                          Price: ₹{note.price} (Seller gets ₹{(note.price * 0.9).toFixed(1)})
                        </span>
                      </div>
                  ...
                  <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <input
                      type="text"
                      placeholder="Optional feedback reason for author..."
                      value={feedbackInput[note.id] || ''}
                      onChange={(e) => setFeedbackInput({ ...feedbackInput, [note.id]: e.target.value })}
                      className="w-full sm:w-80 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs"
                    />
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-bold transition"
                      >
                        Reject
                      </button>
                      <button
                  ...
                  <tr key={note.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        note.status === 'approved' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                      }`}>
                        {note.status}
                      </span>
                    </td>
